"""Concrete retriever for enemy-force knowledge documents.

Implements RetrieverInterface from the existing
backend/modules/knowledge/enemy/interfaces.py contract.
"""
...
logger = logging.getLogger("dss.knowledge_base.enemy.retriever")
...
def _get_default_dataset_path() -> str:
    ...
    entry = get_dataset_registry_entry("enemy_platforms")
    ...
    return str(here / "enemy_platforms.json")


class EnemyKnowledgeRetriever(RetrieverInterface):
    """Retrieves enemy-force platform knowledge from the knowledge base."""
    ...
    def _ensure_loaded(self) -> None:
        if self._loaded:
            return
        logger.info(
            "EnemyKnowledgeRetriever | Loading dataset: %s",
        ...
        for v in [SchemaValidator(), FieldValidator(), DuplicateValidator()]:
            ...
            if not vresult.valid:
                raise RuntimeError(
                    f"Enemy dataset validation failed: {vresult.errors}"
                )
        ...
        self._loaded = True
        logger.info(
            "EnemyKnowledgeRetriever | Loaded %d records | Index created | "
    ...
    def _doc_to_knowledge_item(self, doc: dict[str, Any]) -> KnowledgeItem:
        return KnowledgeItem(
            ...
            threat_indicators=list(doc.get("threat_indicators", [])),
            tactical_role=doc.get("tactical_role"),
            confidence=0.7,
            metadata={
                k: v
        ...
        if self._ontology_service is not None:
            ...
            for term in expanded:
                if term not in queries:
                    queries.append(term)
            logger.info(
                "EnemyKnowledgeRetriever | Ontology expanded '%s' -> %s",
                query,
                queries,
            )
    ...
    async def retrieve(
        ...
        logger.info(
            "EnemyKnowledgeRetriever | Searching Enemy KB | Query: %s",
            query,
        )
        ...
        logger.info(
            "EnemyKnowledgeRetriever | Final knowledge items: %d | "
